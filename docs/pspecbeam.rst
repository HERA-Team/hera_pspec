...
The main purpose of `PSpecBeam` objects is to provide the `PSpecData` class with a way of normalizing the power spectra that it produces, using the :meth:`~hera_pspec.pspecbeam.PSpecBeamBase.compute_pspec_scalar` method. To attach a `PSpecBeam` object to a `PSpecData` object, pass one in when you instantiate the class, e.g.
