...
if rfi_flag:
    # get algorithm parameters
    globals().update(cf['algorithm']['rfi'])
...
if xtalk_sub:
    ...
    def xtalk_sub(j):
        try:
            ...
            xtalk_file = os.path.join(out_dir, xtalk_ext.join(os.path.splitext(df)))
            F.write_data(xtalk_file, write_avg=True, overwrite=overwrite)
            # write xtalk-subtracted data
            out_df = os.path.join(out_dir, df + file_ext)
        ...
        try:
            ...
            for pstokes in outstokes:
                try:
                    ...
                    outfile = dfs[0].replace(".{}.".format(dps[0]), ".{}.".format(pstokes))
