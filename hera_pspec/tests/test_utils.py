...
class Test_Utils(unittest.TestCase):
    ...
    def test_spw_range_from_freqs(self):
        ...
        spw3 = utils.spw_range_from_freqs(
            self.uvd, freq_range=(98e6, 120e6), bounds_error=False
        )
    ...
    def test_spw_range_from_redshifts(self):
        ...
        spw1 = utils.spw_range_from_redshifts(self.uvd, z_range=(7.0, 8.0))
        spw2 = utils.spw_range_from_redshifts(self.uvd, z_range=z_list)
        utils.spw_range_from_redshifts(
            self.uvd, z_range=(12.0, 14.0), bounds_error=False
        )
        utils.spw_range_from_redshifts(self.uvd, z_range=(6.2, 7.2))
...
def test_job_monitor():
    ...
    failures = utils.job_monitor(
        run_func, range(len(datafiles)), "test", maxiter=1, verbose=False
    )
    # assert job 1 failed
    ...
    np.random.seed(0)
    failures = utils.job_monitor(
        run_func, range(len(datafiles)), "test", maxiter=10, verbose=False
    )
