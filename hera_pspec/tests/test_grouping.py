...
from .. import uvpspec, conversions, pspecbeam, pspecdata, testing, utils
...
class Test_grouping(unittest.TestCase):
    ...
    def test_average_spectra(self):
        ...
        uvp.get_exact_window_functions(
            ftbeam_file=os.path.join(DATA_PATH, "FT_beam_HERA_dipole_test"),
            spw_array=None,
            inplace=True,
            verbose=False,
        )
...
def test_validate_bootstrap_errorbar():
    ...
    Nsamples = 100
    seed = 0
    uvp_avg, uvp_boots, uvp_wgts = grouping.bootstrap_resampled_error(
        uvp,
        time_avg=False,
        Nsamples=Nsamples,
        seed=seed,
...
def test_spherical_average():
    ...
    uvp.cov_array_real = {
        s: np.repeat(
            np.repeat(
                np.eye(uvp.Ndlys, dtype=np.float64)[None, :, :, None], uvp.Nblpairts, 0
            ),
            uvp.Npols,
            -1,
        )
    ...
    uvp.stats_array = {
        "err": {
            s: np.ones((uvp.Nblpairts, uvp.Ndlys, uvp.Npols), dtype=np.complex128)
            for s in range(uvp.Nspws)
        }
    }
    ...
    sph = grouping.spherical_average(
        uvp, kbins, bin_widths, add_to_history="checking 1 2 3", A=A
    )
    ...
    for spw in sph.spw_array:
        ...
        assert np.isclose(
            sph.get_kparas(spw), kbins
        ).all()  # assert kbins are input kbins
        assert np.isclose(
            sph.window_function_array[spw].sum(axis=2), 1
        ).all()  # assert window func is normalized
    ...
    sph2 = grouping.spherical_average(
        uvp, kbins * cosmo.h, bin_widths * cosmo.h, little_h=False
    )
    for spw in sph.spw_array:
    ...
    sph_a, sph_b = sph.select(spws=[0], inplace=False), sph.select(
        spws=[1], inplace=False
    )
    ...
    blpair_groups, blpair_lens, _ = uvp.get_red_blpairs()
    sph2 = grouping.spherical_average(
        uvp, kbins, bin_widths, blpair_groups=blpair_groups
    )
...
def test_spherical_wf_from_uvp():
    ...
    pytest.raises(
        AssertionError, grouping.spherical_wf_from_uvp, uvp, kbins, bin_widths
    )
