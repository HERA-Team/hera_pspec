...
def test_select_common():
    ...
    uvp_new = uvputils.select_common(
        uvp_list, spws=True, blpairs=True, times=True, polpairs=True, inplace=False
    )
    assert uvp_new[0] == uvp_new[1]
    np.testing.assert_array_equal(uvp_new[0].time_avg_array, uvp_new[1].time_avg_array)

    # Check that selecting on common baseline-pairs works
    uvp_list_2 = [uvp1, uvp2, uvp3]
    uvp_new_2 = uvputils.select_common(
        uvp_list_2, spws=True, blpairs=True, times=True, polpairs=True, inplace=False
    )
    assert uvp_new_2[0] == uvp_new_2[1]
    assert uvp_new_2[0] == uvp_new_2[2]
    np.testing.assert_array_equal(
        uvp_new_2[0].time_avg_array, uvp_new_2[1].time_avg_array
    )
    ...
    uvp_new = uvputils.select_common(
        uvp_list, spws=True, blpairs=True, times=False, polpairs=True, inplace=False
    )
