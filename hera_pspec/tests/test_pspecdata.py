...
class Test_PSpecData(unittest.TestCase):
    def setUp(self):
        ...
        self.uvd_std.read_miriad(
            os.path.join(DATA_PATH, "zen.2458042.17772.std.xx.HH.uvXA")
        )
    ...
    def test_set_symmetric_taper(self):
        ...
        rmat = dspec.dayenu_mat_inv(
            x=self.ds.freqs[self.ds.spw_range[0] : self.ds.spw_range[1]],
            filter_centers=[0.0],
            filter_half_widths=[100e-9],
            filter_factors=[1e-9],
        )
        wmat = np.outer(
            np.diag(np.sqrt(self.ds.Y(key1))), np.diag(np.sqrt(self.ds.Y(key1)))
        )
        rmat = np.linalg.pinv(wmat * rmat)
        self.assertTrue(
            np.all(np.isclose(rmat_symmetric, bh_taper.T * rmat * bh_taper, atol=1e-6))
        )
        self.assertTrue(np.all(np.isclose(rmat_a, bh_taper.T**2.0 * rmat, atol=1e-6)))
    ...
    def test_get_Q_alt(self):
        ...
        for i in range(vect_length):
            ...
            self.assertAlmostEqual(np.imag(xQx), 0.0)

        x_vect = np.ones(vect_length)
        Q_matrix = self.ds.get_Q_alt(vect_length // 2)
        xQx = np.dot(np.conjugate(x_vect), np.dot(Q_matrix, x_vect))
        self.assertAlmostEqual(xQx, np.abs(vect_length**2.0))
    ...
    def test_get_Q(self):
        ...
        uvd = copy.deepcopy(self.uvd)
        pspecdata.PSpecData(dsets=[uvd, uvd])
    ...
    def test_get_unnormed_E(self):
        ...
        wgt_matrix_dict = (
            {}
        )  # The keys here have no significance except they are formatted right
        ...
        ds_c = pspecdata.PSpecData(
            dsets=[uvd, uvd], wgts=[None, None], labels=["red", "blue"], beam=self.bm
        )
        ds_c.spw_Ndlys = 10
        ...
        self.assertEqual(
            E_matrices.shape, (ds_c.spw_Ndlys, ds_c.spw_Nfreqs, ds_c.spw_Nfreqs)
        )
            ...
            for i in range(ds.spw_Nfreqs):
                for j in range(ds.spw_Nfreqs):
                    if flags1[i] * flags2[j] == 0:  # either channel flagged
                        self.assertAlmostEqual(matrix[i, j], 0.0)
                    else:
    ...
    def test_cross_covar_model(self):
        ...
        pytest.raises(
            ValueError, ds.cross_covar_model, key1, key2, model="other_string"
        )
    ...
    def test_get_MW(self):
        ...
        for mode in ["H^-1", "V^-1/2", "I", "L^-1"]:
            if mode == "H^-1":
                ...
                for i in range(n):
                    self.assertAlmostEqual(W[i, i], 1.0)
                    for j in range(n):
                ...
                for i in range(3):
                    self.assertAlmostEqual(norm[i], 1.0)
                ...
                M, W = self.ds.get_MW(
                    random_G, random_H, mode=mode, band_covar=random_V
                )
                ...
                pytest.raises(
                    NotImplementedError, self.ds.get_MW, random_G, random_H, mode=mode
                )
    ...
    def test_cov_q(self, ndlys=13):
        ...
        for d in self.d:
            ...
            d.select(
                times=np.unique(d.time_array)[:10], frequencies=d.freq_array[0, :16]
            )
        for d_std in self.d_std:
            d_std.flag_array[:] = False
            d_std.select(
                times=np.unique(d_std.time_array)[:10],
                frequencies=d_std.freq_array[0, :16],
            )
        ...
        cov_analytic = np.zeros(
            (self.ds.spw_Ndlys, self.ds.spw_Ndlys), dtype=np.complex128
        )
        for alpha in range(self.ds.spw_Ndlys):
            for beta in range(self.ds.spw_Ndlys):
                cov_analytic[alpha, beta] = np.exp(
                    -2j * np.pi * (alpha - beta) * (chan_x - chan_y) / self.ds.spw_Ndlys
                ).sum()
        key1 = (0, 24, 38)
        key2 = (1, 25, 38)
        # print(cov_analytic)
        ...
        for input_data_weight in ["identity", "iC", "dayenu"]:
            ...
            for _ in taper_selection:
                qc = self.ds.cov_q_hat(key1, key2, model="dsets")
                self.assertTrue(
                    np.allclose(
                        np.array(list(qc.shape)),
                        np.array(
                            [self.ds.Ntimes, self.ds.spw_Ndlys, self.ds.spw_Ndlys]
                        ),
                        atol=1e-6,
                    )
                )
        ...
        self.assertTrue(
            np.allclose(
                qc,
                np.repeat(cov_analytic[np.newaxis, :, :], self.ds.Ntimes, axis=0),
                atol=1e-6,
            )
        )
        """
        ...
        self.assertRaises(
            ValueError, self.ds.cov_q_hat, key1, key2, time_indices="watch out!"
        )
    ...
    def test_cov_p_hat(self):
        ...
        """
        self.ds = pspecdata.PSpecData(dsets=self.d, wgts=self.w, dsets_std=self.d_std)
        cov_p = self.ds.cov_p_hat(
            np.sqrt(6.0) * np.identity(10), np.array([5.0 * np.identity(10)])
        )
        for p in range(10):
            for q in range(10):
                if p == q:
                    self.assertTrue(np.isclose(30.0, cov_p[0, p, q], atol=1e-6))
                else:
                    self.assertTrue(np.isclose(0.0, cov_p[0, p, q], atol=1e-6))
    ...
    def test_q_hat(self):
        ...
        for input_data_weight in ["identity", "iC", "dayenu"]:
            ...
            for taper in taper_selection:
                ...
                self.ds.q_hat(key1, key4)  # q_hat(x1, x2+x2)
                self.ds.q_hat(key4, key1)  # q_hat(x2+x2, x1)
        ...
        for input_data_weight in ["identity", "iC", "dayenu"]:
            ...
            for taper in taper_selection:
                ...
                self.assertTrue(
                    np.isclose(np.imag(q_hat_a / q_hat_a_slow), 0, atol=1e-6).all()
                )
            ...
            for taper in taper_selection:
                ...
                if input_data_weight == "identity":
                    ...
                    self.assertLessEqual(
                        anti_sym_norm, matrix_scale * multiplicative_tolerance
                    )
                    ...
                    self.assertLessEqual(
                        G_diff_norm, matrix_scale * multiplicative_tolerance
                    )
                    ...
                    for i in range(Nfreq - 2):
                        for j in range(Nfreq - 2):
                            self.assertGreaterEqual(
                                G[i, j], -min_diagonal * multiplicative_tolerance
                            )
                else:
    ...
    def test_scalar(self):
        ...
        pspecdata.PSpecData(dsets=self.d, wgts=self.w, beam=gauss)
    ...
    def test_units(self):
        ...
        ds_b = pspecdata.PSpecData(
            dsets=[self.uvd, self.uvd], wgts=[None, None], beam=self.bm
        )
    ...
    def test_get_analytic_covariance(self):
        ...
        for _ in range(2):
            ...
            new.lst_array = uvutils.get_lst_for_time(
                new.time_array, *new.telescope_location_lat_lon_alt_degrees
            )
            uvd += new
    ...
    def test_pspec(self):
        ...
        uvd = copy.deepcopy(self.uvd)
        ds = pspecdata.PSpecData(
            dsets=[uvd, uvd], wgts=[None, None], beam=self.bm, labels=["red", "blue"]
        )
        ...
        ds = pspecdata.PSpecData(dsets=[uvd, uvd], wgts=[None, None], beam=self.bm)
        uvp = ds.pspec(
            bls, bls, (0, 1), ("xx", "xx"), spw_ranges=[(10, 24)], verbose=False
        )
        ...
        ds = pspecdata.PSpecData(
            dsets=[copy.deepcopy(self.uvd), copy.deepcopy(self.uvd)],
            wgts=[None, None],
            beam=self.bm,
        )
    ...
    def test_normalization(self):
        ...
        d1 = self.uvd.select(
            times=np.unique(self.uvd.time_array)[:-1:2],
            frequencies=np.unique(self.uvd.freq_array)[40:51],
            inplace=False,
        )
        d2 = self.uvd.select(
            times=np.unique(self.uvd.time_array)[1::2],
    ...
    def test_broadcast_dset_flags(self):
        ...
        ds.dsets[0].flag_array[
            ds.dsets[0].antpair2ind(24, 25, ordered=False)[3], 0, 600, 0
        ] = True
        ...
        ds.dsets[0].data_array[
            uvd.antpair2ind(24, 25, ordered=False)[3], 0, 400:450, :
        ] *= 100
        ds.pspec(
            [(24, 25), (37, 38), (38, 39)],
            [(24, 25), (37, 38), (38, 39)],
    ...
    def test_validate_blpairs(self):
        ...
        uvd = copy.deepcopy(self.uvd)
        pytest.raises(
            TypeError, pspecdata.validate_blpairs, [((1, 2), (2, 3))], None, uvd
        )
        pytest.raises(
            TypeError, pspecdata.validate_blpairs, [((1, 2), (2, 3))], uvd, None
        )
...
def test_pspec_run():
    ...
    assert not (
        ...
        or ds.dsets[0].get_flags(37, 38)[0, 25:].any()
    )
    ...
    pytest.raises(
        AssertionError, pspecdata.pspec_run, fnames, "./out.h5", beam=1, verbose=False
    )
