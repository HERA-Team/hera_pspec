...
class Test_UVPSpec(unittest.TestCase):
    ...
    def _add_optionals(self, uvp):
        ...
        for spw in uvp.spw_array:
            ...
            uvp.cov_array_real[spw] = np.empty(
                (uvp.Nblpairts, ndlys, ndlys, uvp.Npols), np.float64
            )
            uvp.cov_array_imag[spw] = np.empty(
                (uvp.Nblpairts, ndlys, ndlys, uvp.Npols), np.float64
            )
            uvp.stats_array[stat][spw] = np.empty(
                (uvp.Nblpairts, ndlys, uvp.Npols), np.complex128
            )
        return uvp
    ...
    def test_get_funcs(self):
        ...
        # test omit_flags
        self.uvp.integration_array[0][
            self.uvp.blpair_to_indices(((1, 2), (1, 2)))[:2]
        ] = 0.0
        assert self.uvp.get_integrations(
    ...
    def test_stats_array(self):
        ...
        pytest.raises(
            ValueError, uvp.set_stats, "errors", keys[0], np.linspace(0, 1, 2)
        )
        ...
        folded_errs = np.sum(
            [1 / errs[:, 1:15][:, ::-1] ** 2.0, 1 / errs[:, 16:] ** 2.0], axis=0
        ) ** (-0.5)
    ...
    def test_select(self):
        ...
        uvp2 = uvp.select(
            blpairs=uvp.get_blpairs(), polpairs=uvp.polpair_array, inplace=False
        )
    ...
    def test_get_exact_window_functions(self):
        ...
        uvp.get_exact_window_functions(
            ftbeam_file=self.ft_file, spw_array=0, inplace=True, verbose=True
        )
        ...
        kperp_bins, kpara_bins, wf_array = uvp.get_exact_window_functions(
            ftbeam_file=self.ft_file, inplace=False
        )
    ...
    def test_combine_uvpspec(self):
        ...
        assert out.Npols == 2
        assert len(set(out.polpair_array) ^ {1515, 1414}) == 0
