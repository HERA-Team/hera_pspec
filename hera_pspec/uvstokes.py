...
class UVStokes(object):

   def __init__(self, dsetsxx=[], dsetsxy=[], dsetsyx=[], dsetsyy=[],combine=None):
      """
      ...
      combine :  Boolean. Enable to combine all polarization into a single UVData object. Default: None
      """

      self.dsetsxx = dsetsxx
      self.dsetsxx = dsetsxy
      self.dsetsyx = dsetsyx
      self.dsetsyy = dsetsyy

      self.combine = combine

   def construct_stokes():
      """
      Reads miriad files and construct Stokes visibilities. It can return individual UVData objects with Stokes I,Q,U,V visibilities or a single UVData object containing all four Stokes visibilities
      """
      assert (self.dsetsxx==self.dsetsyy)
      assert (self.dsetsxy==self.dsetsyx)

      for idset, dsetxx in enumerate(self.dsetsxx):
         uvxx = pyuvdata.UVData()
         uvxy = pyuvdata.UVData()
         uvyx = pyuvdata.UVData()
         uvyy = pyuvdata.UVData()

         uvxx.read_miriad(dsetxx)
         uvxy.read_miriad(self.dsetsxy[idset])
         uvyx.read_miriad(self.dsetsyx[idset])
         uvyy.read_miriad(self.dsetsyy[idset])

         # extracting data array from the UVData objects
         dataxx = uvxx.data_array
         dataxy = uvxy.data_array
         datayx = uvyx.data_array
         datayy = uvyy.data_array

         # extracting flag array from the UVdata objects
         flagxx = uvxx.flag_array
         flagxy = uvxy.flag_array
         flagyx = uvyx.flag_array
         flagyy = uvyy.flag_array

         # constructing flags
         flagI = flagxx + flagyy # flagQ is the same as flagI
         flagU = flagxy + flagyx # flagV is the same as flagU

         # constructing Stokes visibilities
         dataI = 0.5* (dataxx + datayy)
         dataQ = 0.5* (dataxx - datayy)
         dataU = 0.5* (dataxy + datayx)
         dataV = 0.5j* (datayx - dataxy)

         dataI = np.ma.masked_array(dataI,flagI)
         dataQ = np.ma.masked_array(dataQ,flagI)
         dataU = np.ma.masked_array(dataU,flagU)
         dataV = np.ma.masked_array(dataV,flagU)

         # assigning Stokes visibilities to UVData objects
         uvxx.data_array = dataI
         uvyy.data_array = dataQ
         uvxy.data_array = dataU
         uvyx.data_array = dataV

         # changing the metadata for polarization
         uvxx.polarization_array=1 # to Stokes I
         uvyy.polarization_array=2 # to Stokes Q
         uvxy.polarization_array=3 # to Stokes U
         uvyx.polarization_array=4 # to Stokes V

         # writing individual polarization to single files which are then combine into a sigle UVData object
         outI = dsetxx.replace('xx','I')
         outQ = dsetxy.replace('xx','Q')
         outU = dsetyx.replace('xx','U')
         outV = dsetyy.replace('xx','V')

         uvxx.write_miriad(outI)
         uvyy.write_miriad(outQ)
         uvxy.write_miriad(outU)
         uvyx.write_miriad(outV)

         # returns UVData object which has all four Stokes polarizations
         if self.combine:
            outfile = dsetxx.replace('xx','')
            uv = pyuvdata.UVData()
            uv.read_miriad([outI,outQ,outU,outV])
            uv.write(outfile)
...
class PSpecData(object):
    ...
    def pspec(self, bls1, bls2, dsets, input_data_weight='identity', norm='I', taper='none', 
        ...
        dsets : length-2 tuple or list
            ...
            is for the Left-Hand dataset and second index is used for the Right-Hand dataset (see above).
