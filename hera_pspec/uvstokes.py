...
class Stokes(object):
   def __init__(self,dsetsxx=[],dsetsxy=[],dsetsyx=[],dsetsyy=[],wgtsxx=[],wgtsxy=[],wgtsyx=[],wgtsyy=[],spar=None):
      """
      Object to store multiple sets of Miriad files required to construct Stokes parameters

      Parameters
      ----------
      dsetsxx: List of Miriad files for xx polarization, optional
        List of Miriad files containing data for xx polarization needed to construct the desired polarization. Default: Empty list

      dsetsxy: List of Miriad objects for xy polarization, optional
        List of Miriad files containing data for xy polarization needed to construct the desired polarization. Default: Empty list

      dsetsyx: List of Miriad files for yx polarization, optional
        List of Miriad files containing data for yx polarization needed to construct the desired polarization. Default: Empty list

      dsetsyy: List of Miriad files for yy polarization, optional
        List of Miriad files containing data for yy polarization needed to construct the desired polarization. Default: Empty list
      
      wgtsxx: List of UVData objects, optional
            List of UVData objects containing weights for the xx data.Default: Empty list.
      wgtsxy: List of UVData objects, optional
            List of UVData objects containing weights for the xy data.Default: Empty list.

      wgtsyx: List of UVData objects, optional
            List of UVData objects containing weights for the yx data.Default: Empty list.

      wgtsyy: List of UVData objects, optional
            List of UVData objects containing weights for the yy data.Default: Empty list.
      
      spar : Stokes parameter
        Desired Stokes parameters, can be 'I','Q','U','V'. Default : None
      """
      
      self.dsetsxx = dsetsxx
      self.dsetsxy = dsetsxy
      self.dsetsyx = dsetsyx
      self.dsetsyy = dsetsyy

      # Store input UVData if specified
      if len(dsetsxx) > 0:
         self.add(dsetsxx, wgtsxx)

      if len(dsetsxy) > 0:
         self.add(dsetsxy,wgtsxy)

      if len(dsetsyx) > 0:
         self.add(dsetsyx, wgtsyx)

      if len(dsetsyy) > 0:
         self.add(dsetsyy, wgtsyy)
   ...
   def miriad2pyuvdata(self,dsets):
      """
      Convert Miriad files to UVData objects
   
      Parameters
      ----------
      dsets : List of Miriad files
      """
      uvs = [pyuvdata.UVData(dset) for dset in dsets]
      return [uv.read_miriad(UV) for uv in uvs]
   ...
   def add(self, dsets, wgts):
      ...
      dsets : UVData or list
        UVData object or list of UVData objects containing data to add to the collection.

      wgts : UVData or list
        UVData object or list of UVData objects containing weights to add to the collection. Must be the same length as dsets. If a weight is set to None, the flags of the corresponding
      """
...
# subclass to form Stokes parameters   
class StokesData(Stokes):
   def __init__(self,dsetsxx=[],dsetsxy=[],dsetsyx=[],dsetsyy=[],wgtsxx=[],wgtsxy=[],wgtsyx=[],wgtsyy=[],spar=None)
      Stokes.__init__(self,dsetsxx=[],dsetsxy=[],dsetsyx=[],dsetsyy=[],wgtsxx=[],wgtsxy=[],wgtsyx=[],wgtsyy=[],spar=None)
   
   def corr_output(self):
      """
      Return correlator outputs required to form the desired Stokes parameter
      """
      if self.spar=='I': return 'XX','YY'
      if self.spar=='Q': return 'XX','YY'
      if self.spar=='U': return 'XY','YX'
      if self.spar=='V': return 'XY','YX'   

   def extract_pol(self,dsets=[]):
      """
      Extracts polarization of the Miriad files
      """
      uv = [aipy.miriad.UV(d) for d in dsets]
      pols = [uvi['pol'] for uvi in uv]
      assert np.unique(pols)==1

      return np.unique(pols)

   def extract_datasets(self):
      """
      Validate UVData objects for Stokes parameters, checks if the input UVData objects are the right ones to form the desired Stokes parameter and returns the UVData objects.
      """
      if self.par=='I' or self.par=='Q':
         assert len(self.dsetsxx)>0
         assert len(self.dsetsyy)>0
         return self.dsetsxy,self.dsetsxy,self.wgtsxx,self.wgtsyy

      if self.par=='Q' or self.par=='U':
         assert len(self.dsetsxy)>0
         assert len(self.dsetsyx)>0
         return self.dsetsxy,self.dsetsxy,self.wgtsxy,self.wgtsyx

   def combinepol(uv,p,d,f):
      """
      Combines correlator outputs
      """
      if p[2] in d2.keys():
         data = d2[p[2]][pol_wgts[self.spar][self.pol2]]
         flags = f2[p[2]][pol_wgts[self.spar][self.pol2]]
         data = d2[n.where(t2['times'] == p[1])[0][0]] #time index to grab from d2
         flags = flags[n.where(t2['times'] == p[1])[0][0]]
         d = 0.5*(d*pol_wgts[self.spar][self.pol1] + data*pol_wgts[self.spar][self.pol2])
         f = f + flags
         d = n.ma.masked_array(d,f)
      return p,d,f  
 
   def construct_stokes(self):
      """
      Combine visibilities to form Stokes parameters
      """
      # weight required to construct Stokes parameters
      pol_wgts = {'I': {'XX': 1. , 'YY': 1. },
                  'Q': {'XX': 1. , 'YY':-1. },
                  'U': {'XY': 1. , 'YX': 1. },
                  'V': {'XY':-1.j, 'YX': 1.j},
                 }

      dsets1,dsets2,wgts1,wgts2 = self.extract_datasets()      
      assert(len(dsets1) == len(dsets2))
      pols = self.corr_output()
      self.pol1 = extract_pol(dsets1) 
      self.pol2 = extract_pol(dsets2) 
      assert self.pol1 in pols
      assert self.pol2 in pols
  
      # outfile
      outfile = dset1.replace(self.pol1,self.spar) ]

      uvi = aipy.miriad.UV(dsets1)
      t2,d2,f2 = miriad_read_file(dsets2,antstr='cross',polstr=self.pol2)
      uvo = a.miriad.UV(outfile, status='new')
      uvo.init_from_uv(uvi, override={'pol':self.spar)
      uvo.pipe(uvi1, mfunc=combinepol, raw=True, append2hist='combine_pol:' + ' '.join(sys.argv) + '\n')
      del(uvi); del(uvo)      
    
class PSpecData(object):
   def __init__(self,dsetsxx=[],dsetsxy=[],dsetsyx=[],dsetsyy=[],wgtsxx=[],wgtsxy=[],wgtsyx=[],wgtsyy=[],spar=None,beam=None)
      Stokes.__init__(self,dsetsxx=[],dsetsxy=[],dsetsyx=[],dsetsyy=[],wgtsxx=[],wgtsxy=[],wgtsyx=[],wgtsyy=[],spar=None)

      # Store a primary beam
      self.primary_beam = beam

   def validate_uvobjects(self,dsets=[],wgts=[]):
      """
