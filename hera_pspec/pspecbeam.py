...
class PSpecBeamUV(PSpecBeamBase):
    ...
    def beam_normalized_response(self, pol='pI', freq=None, x_orientation=None):
        ...
        if pol in pol_array:
            ...
            raise ValueError('Do not have the right polarization information, pol must be in %s. Provided %s.'%(pol_array, pol))
...
class PSpecBeamFromArray(PSpecBeamBase):
    ...
    def __init__(self, OmegaP, OmegaPP, beam_freqs, cosmo=None, x_orientation=None):
        ...
        # python no longer likes when you iterate over changing lists of keys.
        ompkeys = list(OmegaP.keys())
        for key in ompkeys:
