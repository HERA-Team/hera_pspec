...
class Test_PSpecData(unittest.TestCase):
    ...
    def test_lst_avg_wrapping(self):
        """Test that LST averaging uses circular mean to handle 2pi wrapping."""
        # Create a simple PSpecData object
        uvd = copy.deepcopy(self.uvd)
        ds = pspecdata.PSpecData(dsets=[uvd, uvd], wgts=[None, None], beam=self.bm)

        # Test case 1: LST values near 2pi wrap
        # lst_1 near 2pi (6.2 rad), lst_2 just after wrap (0.1 rad)
        # Correct circular mean should be near 0 or 2pi, NOT near pi (3.14)
        lst1_array = np.array([6.2, 6.25, 6.28])  # near 2*pi (≈6.283)
        lst2_array = np.array([0.01, 0.05, 0.08])  # just after wrapping

        # Compute circular mean (what the code should do)
        angles = np.array([lst1_array, lst2_array])
        correct_avg = np.angle(np.mean(np.exp(1j * angles), axis=0)) % (2*np.pi)

        # The average should be close to either 0 or 2pi (near the wrap point)
        # NOT near pi (which would be the buggy arithmetic mean)
        # Check that all values are either < 0.5 or > 5.5 (avoiding the pi region)
        assert np.all((correct_avg < 0.5) | (correct_avg > 5.5)), \
            f"Circular mean should avoid pi region, got {correct_avg}"

        # The buggy arithmetic mean would give ~3.1 (near pi)
        buggy_avg = np.mean([lst1_array, lst2_array], axis=0)
        assert np.all(buggy_avg > 2.5) and np.all(buggy_avg < 3.5), \
            "This verifies the buggy version would give wrong result near pi"

        # Test case 2: LST values NOT near wrap (should give same result either way)
        lst1_no_wrap = np.array([1.0, 1.5, 2.0])
        lst2_no_wrap = np.array([1.1, 1.6, 2.1])

        angles_no_wrap = np.array([lst1_no_wrap, lst2_no_wrap])
        circular_avg = np.angle(np.mean(np.exp(1j * angles_no_wrap), axis=0)) % (2*np.pi)
        arithmetic_avg = np.mean([lst1_no_wrap, lst2_no_wrap], axis=0)

        # When not wrapping, both methods should agree
        assert np.allclose(circular_avg, arithmetic_avg, atol=1e-10), \
            "Circular and arithmetic mean should agree when not wrapping"

        # Test case 3: Verify actual pspec creates correct lst_avg_array
        # Manually create a simple pspec-like scenario
        uvd1 = copy.deepcopy(self.uvd)
        uvd2 = copy.deepcopy(self.uvd)

        # Modify LST arrays to test wrapping
        # Get unique times and set LSTs to wrap scenario
        times = np.unique(uvd1.time_array)[:3]  # Use first 3 times
        uvd1 = uvd1.select(times=times, inplace=False)
        uvd2 = uvd2.select(times=times, inplace=False)

        # Set LST arrays to create wrapping scenario
        uvd1.lst_array[:] = 6.25  # near 2pi
        uvd2.lst_array[:] = 0.05  # just after wrap

        ds_wrap = pspecdata.PSpecData(dsets=[uvd1, uvd2], wgts=[None, None], beam=self.bm)

        # Run pspec
        bls = [(24, 25), (37, 38)]
        bls1, bls2, blpairs = utils.construct_blpairs(bls, exclude_permutations=False,
                                                      exclude_auto_bls=True, group=True)
        uvp = ds_wrap.pspec(bls1, bls2, (0, 1), [('xx','xx')], spw_ranges=[(20, 30)],
                           input_data_weight='identity', norm='I', taper='none',
                           verbose=False)

        # Check that lst_avg_array is near 0 or 2pi, NOT near pi
        assert np.all((uvp.lst_avg_array < 0.5) | (uvp.lst_avg_array > 5.5)), \
            f"UVPspec lst_avg_array should use circular mean, got {uvp.lst_avg_array}"
