...
class TestGrouping:
    ...
    def test_lst_avg_wrapping_in_average_spectra(self):
        """
        Test that average_spectra with time_avg=True uses circular mean for LST wrapping.
        """
        # Start with basic file setup
        dfile = os.path.join(DATA_PATH, 'zen.all.xx.LST.1.06964.uvA')
        uvd = UVData()
        uvd.read_miriad(dfile)
        cosmo = conversions.Cosmo_Conversions()
        ...
        # Create two datasets with LST values that wrap around 2pi
        uvd1 = uvd.select(times=np.unique(uvd.time_array)[::2], inplace=False)
        uvd2 = uvd.select(times=np.unique(uvd.time_array)[1::2], inplace=False)

        # Manually set LST arrays to create wrapping scenario
        # Use first 4 times from each
        times1 = np.unique(uvd1.time_array)[:4]
        times2 = np.unique(uvd2.time_array)[:4]
        uvd1 = uvd1.select(times=times1, inplace=False)
        uvd2 = uvd2.select(times=times2, inplace=False)

        # Create wrapping LST scenario for different time pairs
        # Set uvd1 LSTs near 2pi, uvd2 LSTs just after wrap
        for i, t in enumerate(times1):
            mask = uvd1.time_array == t
            uvd1.lst_array[mask] = 6.2 + i * 0.02  # 6.2, 6.22, 6.24, 6.26 (all near 2pi≈6.283)

        for i, t in enumerate(times2):
            mask = uvd2.time_array == t
            uvd2.lst_array[mask] = 0.05 + i * 0.02  # 0.05, 0.07, 0.09, 0.11 (just after wrap)

        # Create PSpecData and run pspec
        ds = pspecdata.PSpecData(dsets=[uvd1, uvd2], wgts=[None, None], beam=uvb)
        ds.dsets[0].vis_units = 'mK'
        ds.dsets[1].vis_units = 'mK'
        ...
        uvp = ds.pspec(baselines1, baselines2, (0, 1), [('xx', 'xx')],
                      spw_ranges=[(300, 350)],
                      input_data_weight='identity',
                      norm='I',
                      taper='none',
        ...
        # Check that uvp.lst_avg_array uses circular mean (should be near 0 or 2pi, not pi)
        assert np.all((uvp.lst_avg_array < 1.0) | (uvp.lst_avg_array > 5.5)), \
            f"Initial UVPspec should use circular mean for lst_avg_array, got {uvp.lst_avg_array}"

        # Now test average_spectra with time_avg=True
        blpair_groups = [blpairs]
        uvp_avg = grouping.average_spectra(uvp,
                                          blpair_groups=blpair_groups,
                                          time_avg=True,
        ...
        # The averaged LST values should still be near 0 or 2pi, not pi
        # Since we're averaging times that are already near the wrap point
        assert np.all((uvp_avg.lst_avg_array < 1.0) | (uvp_avg.lst_avg_array > 5.5)), \
            f"average_spectra should use circular mean for LST, got {uvp_avg.lst_avg_array}"

        # Verify the same for lst_1_array and lst_2_array
        assert np.all((uvp_avg.lst_1_array < 1.0) | (uvp_avg.lst_1_array > 5.5)), \
            f"average_spectra should use circular mean for lst_1_array, got {uvp_avg.lst_1_array}"
        assert np.all((uvp_avg.lst_2_array < 1.0) | (uvp_avg.lst_2_array > 5.5)), \
            f"average_spectra should use circular mean for lst_2_array, got {uvp_avg.lst_2_array}"
