...
for i in range(len(data_cfg["subdirs"])):
    files += glob.glob(
        os.path.join(data_cfg["root"], data_cfg["subdirs"][i], data_cfg["template"])
    )
for f in files:
    print(f)
...
for key in pspec_defaults.keys():
    if key in pspec_cfg.keys():
        pspec_defaults[key] = pspec_cfg[key]
...
for i in dset_idxs:
    for j in dset_idxs:
        ...
        ps_store.set_pspec(
            group=pspec_cfg["groupname"],
            psname=pspec_name,
            pspec=ps,
            overwrite=pspec_defaults["overwrite"],
        )
