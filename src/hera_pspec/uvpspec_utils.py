            ...
            for k, blp in enumerate(blps1):
                ...
                if hasattr(uvp1, "window_function_array") and hasattr(
                    ...
                    uvp1.window_function_array[i][blp1_inds, ..., j] = np.sqrt(
                        np.abs(window1) ** 2 + np.abs(window2) ** 2
                    )
...
def _get_blpairs_from_bls(uvp, bls, only_pairs_in_bls=False):
    ...
    if only_pairs_in_bls:
        blp_select = np.array(
            [bool((blp[0] in bls) * (blp[1] in bls)) for blp in blpair_bls]
        )
