...
def _combine_pol(uvd1, uvd2, pol1, pol2, pstokes="pI", x_orientation=None):
    ...
    for i, uvd in enumerate([uvd1, uvd2]):
        ...
        if pol_convention is None:
            warnings.warn(
                f'No polarization convention in uvd{i + 1}. Considering it to be "avg".'
            )
    ...
    uvdS.history = (
        f"Merged into pseudo-stokes vis with hera_pspec version {__version__}\n"
        + "-" * 20
        + f"\ndset1 history:\n{uvd1.history}\n"
        + "-" * 20
...
def construct_pstokes(
    ...
    if isinstance(pstokes, str):
        pstokes = pyuvdata.utils.polstr2num(
            pstokes, x_orientation=dset1.telescope.get_x_orientation_from_feeds()
        )
...
def filter_dset_on_stokes_pol(dsets, pstokes):
    ...
    if isinstance(pstokes, str):
        pstokes = pyuvdata.utils.polstr2num(
            pstokes, x_orientation=dsets[0].telescope.get_x_orientation_from_feeds()
        )
