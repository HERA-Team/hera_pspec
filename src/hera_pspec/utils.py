...
def variance_from_auto_correlations(uvd, bl, spw_range, time_index):
    ...
    assert isinstance(spw_range, tuple) and len(spw_range) == 2, (
    ...
    nsample_bl = np.where(
        nsample_bl > 0,
        nsample_bl,
        (np.median(nsample_bl[nsample_bl > 0]) if np.any(nsample_bl > 0) else 0),
    )
    ...
    var = np.where(
        nsample_bl == 0, np.inf, np.abs(x_bl1 * x_bl2.conj()) / dt / df / nsample_bl
    )
...
def construct_blpairs(
    ...
    assert (not exclude_auto_bls) or (not exclude_cross_bls), (
...
def calc_blpair_reds(
    ...
    if filter_blpairs and uvd1.flag_array is not None and uvd2.flag_array is not None:
        xants1, xants2 = (
            set(uvd1.telescope.antenna_numbers),
            set(uvd2.telescope.antenna_numbers),
        )
...
def uvp_noise_error(
    ...
    for spw in uvp.spw_array:
        ...
        for blp in uvp.get_blpairs():
            ...
            times_blp_1 = auto_Tsys.time_array[
                auto_Tsys.antpair2ind(blp[0][0], blp[0][0])
            ]
            times_blp_2 = auto_Tsys.time_array[
                auto_Tsys.antpair2ind(blp[1][0], blp[1][0])
            ]
            ...
            tinds1 = [
                np.where(np.isclose(times_blp_1, t, atol=1e-5, rtol=0))[0][0]
                for t in time_1_selection
            ]
            tinds2 = [
                np.where(np.isclose(times_blp_2, t, atol=1e-5, rtol=0))[0][0]
                for t in time_2_selection
            ]
            ...
            for polpair in uvp.polpair_array:
                ...
                if precomp_P_N is None:
                    ...
                    if np.all(Tflag1 | Tflag2):
                        ...
                        # get weights
                        Tsys1 = np.sum(Tsys1 * ~Tflag1 * taper, axis=-1) / np.sum(
                            ~Tflag1 * taper, axis=-1
                        ).clip(1e-20, np.inf)
                        Tsys2 = np.sum(Tsys2 * ~Tflag2 * taper, axis=-1) / np.sum(
                            ~Tflag2 * taper, axis=-1
                        ).clip(1e-20, np.inf)
                        Tflag1 = np.all(Tflag1, axis=-1)
                        Tflag2 = np.all(Tflag2, axis=-1)
                        # interpolate to appropriate LST grid
                        if (np.count_nonzero(~Tflag1) > 1) & (
                            np.count_nonzero(~Tflag2) > 1
                        ):
                            Tsys1 = interp1d(
                                lsts[tinds1][~Tflag1],
                                Tsys1[~Tflag1],
                            ...
                            Tsys2 = interp1d(
                                ...
                                bounds_error=False,
                                fill_value="extrapolate",
                            )(uvp.lst_2_array[blp_inds])
                            Tsys = (Tsys1 * Tsys2) ** 0.5
                        else:
                            Tsys = (Tsys1[0] * Tsys2[0]) ** 0.5
...
def get_Q_alt(
    ...
    if mode >= n_delays or mode < 0:
        raise IndexError(
            "Cannot compute Q matrix for a mode outsideof allowed range of delay modes."
        )
    if n_extend:
