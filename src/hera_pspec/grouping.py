...
def average_spectra(
    ...
    if len(set(all_blpairs)) < len(all_blpairs):
        print(
            "Warning: some baseline-pairs are repeated between blpair averaging groups."
        )
    ...
    uvp.Ntpairs = len(
        set(
            (np.round(t1 / time_tol, 0), np.round(t2 / time_tol, 0))
            for t1, t2 in zip(time_1, time_2)
        )
    )
    ...
    uvp.history = "Spectra averaged with hera_pspec [{}]\n{}\n{}\n{}".format(
        __version__, add_to_history, "-" * 40, uvp.history
    )
    # Validity check
    uvp.check()
...
def spherical_average(
    ...
    if store_window:
        ...
        if kbins_theory is None:
            ...
            bin_widths_theory = (
                np.ones_like(kbins_theory) * np.diff(kbins_theory).mean()
            )
            # transform kgrid to little_h units
            if not little_h:
            ...
            assert np.all(kbin_left_theory[1:] >= kbin_right_theory[:-1] - 1e-6), (
                "kbins_theory must not overlap"
            )
    ...
    for spw in uvp.spw_array:
        ...
        if store_window:
            if uvp.exact_windows:
                window_function_array[spw] = np.zeros(
                    (uvp.Ntimes, Nk, Nk_theory, uvp.Npols), dtype=np.float64
                )
            else:
                Nkpar = uvp.window_function_array[spw].shape[2]
                window_function_array[spw] = np.zeros(
                    (uvp.Ntimes, Ndlyblps, Nkpar, uvp.Npols), dtype=np.float64
                )
        ...
        for b, blp in enumerate(uvp.get_blpairs()):
            ...
            if store_window and not uvp.exact_windows:
                window_function_array[spw][:, dslice, ...] = uvp.window_function_array[
                    spw
                ][blpt_inds]
...
def spherical_wf_from_uvp(
    ...
    if little_h != ("h^-3" in uvp_in.norm_units):
        warnings.warn(
            "Changed little_h units to make kbins consistent "
    ...
    for spw in spw_array:
        ...
        # setup arrays
        window_function_array[spw] = np.zeros(
            (uvp.Ntimes, Nk, Nk_in, uvp.Npols), dtype=np.float64
        )
        ...
        for ip, polpair in enumerate(uvp.polpair_array):
            ...
            for it in range(uvp.Ntimes):
                ...
                for m1 in range(Nk):
                    ...
                    if np.any(mask1):
                        wf_temp = np.sum(
                            cyl_wf[it, ...] * mask1[:, :, None, None].astype(int),
                            axis=(0, 1),
                        ) / np.sum(mask1)
                        if np.sum(wf_temp) > 0.0:
                            for m2 in range(Nk_in):
                                mask2 = (kbin_edges_theory[m2] <= ktot) & (
                                    ktot < kbin_edges_theory[m2 + 1]
                                )
                                if np.any(
                                    mask2
    ...
    for spw in range(uvp.Nspws):
        ...
        if Ndlys % 2 == 0:
            ...
            if hasattr(uvp, "cov_array_real"):
                ...
                uvp.cov_array_real[spw][:, Ndlys // 2 + 1 :, Ndlys // 2 + 1 :, :] = (
                    0.25 * (leftleft + leftright + rightleft + rightright)
                )
                ...
                uvp.cov_array_imag[spw][:, Ndlys // 2 + 1 :, Ndlys // 2 + 1 :, :] = (
                    0.25 * (leftleft + leftright + rightleft + rightright)
                )
...
def bootstrap_resampled_error(
    ...
    # type check
    assert isinstance(uvp, (UVPSpec, str)), (
    ...
    uvp_avg.history = (
        ...
        "".format(
            __version__, Nsamples, seed, add_to_history, "-" * 40, uvp_avg.history
        )
    )
...
def _bin_data_like_array(
    ...
    for i, (slc, kernel) in enumerate(zip(slices, kernels)):
        out.swapaxes(-1, axis)[..., i] = np.sum(
            d.swapaxes(-1, axis)[..., slc] * kernel, axis=-1
        )
...
def _bin_cov_like_array(
    ...
    for i, (k1, slc) in enumerate(zip(kernels, slices)):
        for j, (k2, slc2) in enumerate(zip(kernels, slices)):
            ...
            newcov[:, i, j] = np.sum(
                cov[:, slc, slc2] * kernel2d[None, :, :, None], axis=(1, 2)
            )
    return newcov


def _get_delay_slices(
    dly: np.ndarray, kernel: np.ndarray, zero_kernel: np.ndarray = np.ones(1)
    ...
    return (
        neg_slices[::-1]
        + [slice(zero_idx - nzero // 2, zero_idx + 1 + nzero // 2)]
        + pos_slices,
...
def average_in_delay_bins(
    ...
    if hasattr(uvp, "cov_array_real"):
        cov_weighted_stats = tuple(
            stat for stat in cov_weighted_stats if stat in stats_array
        )
    else:
        cov_weighted_stats = ()
    ...
    if nk > len(uvp.dly_array) // 2:
        raise ValueError(
            "The kernel size must be smaller than half the size of the delay array!"
        )
    ...
    for spw in uvp.spw_array:
        ...
        slices, kernels = _get_delay_slices(
            dly, kernel=kernel, zero_kernel=zero_bin_kernel
        )
        newn = len(slices)
        ...
        if hasattr(uvp, "window_function_array"):
            ...
            new_uvp.window_function_array[spw] = _bin_data_like_array(
                oldwf, kernels, slices, axis=1
            )
        ...
        for stat in cov_weighted_stats:
            ...
            diag = np.sqrt(
                np.diagonal(uvp.cov_array_real[spw], axis1=1, axis2=2)
            ).transpose((0, 2, 1))
            ...
            thiscov = (
                corr
                * uvp.stats_array[stat][spw][:, :, None]
                * uvp.stats_array[stat][spw][:, None]
            )
            newc = _bin_cov_like_array(thiscov, kernels, slices)
            stats[stat] = np.diagonal(np.sqrt(newc), axis1=1, axis2=2).transpose(
                (0, 2, 1)
            )
